...
//			if (n == 12 || n == 0x15) v = 0;
			if (n >= 0x03 && n < 15 && drv->raw_track_data[drv->raw_index + 1] == 0x95) data->sync = 1;
			if (n >= 16 && n < 0x17 && drv->raw_track_data[drv->raw_index + 1] == 0x6A) data->sync = 1;
//			x |= v;
		} else {
			if (drv->use_prolog) {
				if (drv->disk_index==12) data->sync = 1;
			} else {
				if (drv->disk_index<=0) data->sync = 1;
			}
		}
//		data->state.rd=x;
	}
