...
{{
	...
	0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x99,0x91,0x92,'-', 0x93,0x94,0x9C,'+', 0x8A,
...
int _decode_key(unsigned long keydata, struct KEYMAP*map)
{
	...
	if (ext) {
		return map->data[KEYMAP_EXT][scan];
	}
	if (GetKeyState(VK_CONTROL)&0x8000) {
		return map->data[KEYMAP_CTRL][scan];
	} else if (GetKeyState(VK_SHIFT)&0x8000) {
		return map->data[KEYMAP_SHIFT][scan];
	} else {
		return map->data[KEYMAP_NORMAL][scan];
	}
}
