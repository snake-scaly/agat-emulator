...
int select_ram(HWND hpar, TCHAR fname[CFGSTRLEN])
{
	...
	ofn.lStructSize = sizeof(ofn);
	ofn.hwndOwner = hpar;
	ofn.lpstrFilter = localize_str(LOC_CFG, 18, buf[0], sizeof(buf[0])); //TEXT("Файлы ПЗУ (*.rom)\0*.rom\0Все файлы\0*.*\0");
	repl_at(buf[0]);
	ofn.lpstrFile = fname;
	ofn.nMaxFile = CFGSTRLEN;
	ofn.lpstrTitle = localize_str(LOC_CFG, 19, buf[1], sizeof(buf[1])); //TEXT("Выбор файла ПЗУ");
	GetCurrentDirectory(MAX_PATH, path);
//	GetModuleFileName(NULL, path, MAX_PATH);
//	PathRemoveFileSpec(path);
	ofn.lpstrInitialDir = path;
