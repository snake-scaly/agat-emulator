...
int get_slot_comment(struct SLOTCONFIG*c, TCHAR*buf)
{
	...
	case DEV_6502:
	case DEV_M6502:
		wsprintf(buf, TEXT("Частота %i%%"), c->cfgint[CFG_INT_CPU_SPEED]);
		return 0;
	case DEV_TAPE_FILE:
		if (c->cfgstr[CFG_STR_TAPE][0])
			_tcscpy(buf, c->cfgstr[CFG_STR_TAPE]);
		else
			_tcscpy(buf, TEXT("Запрос при обращении"));
		wsprintf(buf + _tcslen(buf), TEXT(", %i Гц"), c->cfgint[CFG_INT_TAPE_FREQ]);
		return 0;
	case DEV_SYSTEM:
		switch (c->slot_no) {
		case CONF_MEMORY:
