...
//		printf("thunderclock: mode = %x\n", tcs->cmode);
		switch (tcs->cmode & 0x30) {
		case 0x10: // read
			read_clock(tcs);
//			printf("thunderclock: get time %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x\n", 
...
{
//	printf("thunderclock i/o write: %04X, %02X\n", adr, data);
	adr &= 0x0F;
	switch (adr) {
	case 0:
...
//		system_command(tcs->st->sr, SYS_COMMAND_DUMPCPUREGS, 0, 0);
		...
		if (tcs->nread&1) r |= 0x40;
		r |= tcs->was_int;
//		printf("thunderclock: read bit: %i: %i: %x\n", tcs->ind, (tcs->data[b]>>s)&1, r);
		return r;
...
{
//	printf("thunderclock i/o read: %04X\n", adr);
	adr &= 0x0F;
	switch (adr) {
	case 0:
