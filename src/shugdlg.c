...
static int dialog_init(HWND hwnd, struct SLOTCONFIG*sc)
{
	...
	CheckDlgButton(hwnd, IDC_DRV1, (sc->cfgint[CFG_INT_DRV_TYPE1] == DRV_TYPE_SHUGART)?BST_CHECKED:BST_UNCHECKED);
	CheckDlgButton(hwnd, IDC_DRV2, (sc->cfgint[CFG_INT_DRV_TYPE2] == DRV_TYPE_SHUGART)?BST_CHECKED:BST_UNCHECKED);
...
static int dialog_ok(HWND hwnd, struct SLOTCONFIG*sc)
{
	...
	sc->cfgint[CFG_INT_DRV_TYPE1] = (IsDlgButtonChecked(hwnd, IDC_DRV1)==BST_CHECKED)? DRV_TYPE_SHUGART: DRV_TYPE_NONE;
	sc->cfgint[CFG_INT_DRV_TYPE2] = (IsDlgButtonChecked(hwnd, IDC_DRV2)==BST_CHECKED)? DRV_TYPE_SHUGART: DRV_TYPE_NONE;
...
int shugcfgdlg_run(HWND hpar, struct SLOTCONFIG*sc)
{
	return dialog_run(&dialog, MAKEINTRESOURCE(IDD_SHUGCFG), hpar, sc);
}
